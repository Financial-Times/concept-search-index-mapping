...
## Things to do after the reindexing process
Take a look into Splunk for entries that could be generated by the indexing (the old index is read-only during the process). You can use the query of the `UPP Prod - Concept RW Elasticsearch - Write Errors` alert for finding if there is any problem. If you find entries that failed to be saved in ES, for every one of them you should perform the following procedure:

1) Check lastModified value using the following command:
`curl -v -X GET https://<environment>.ft.com/__concept-rw-elasticsearch/<conceptType>/<uuid>`
2) If lastModified is truly older than it should be:
`curl -v -X GET https://<environment>.ft.com/__aggregate-concept-transformer/concept/<uuid>`
3) Publish the result received from aggregate-concept-transformer to concept-rw-elasticsearch
`curl -v -X PUT https://<environment>.ft.com/__concept-rw-elasticsearch/<conceptType>/<uuid> --data '<payload_from_aggregate-concept-transformer>'`
...
* Credentials(AwsAccessKey, AwsSecretAccessKey) - for local testing only: get them from LastPass -> `AWS ElasticSearch - Provisioning Setup`. When deployed to the k8s clusters, the service uses the IAM role mapped to `serviceAccountName` from the app configuration
